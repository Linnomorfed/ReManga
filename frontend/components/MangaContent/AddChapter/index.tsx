...
const AddChapters: React.FC<AddChaptersProps> = ({ mangaId, toggleVisibility, state, setState }) => {
  ...
  return (
    <ModalBlock variant="large" toggleModalVisibility={toggleVisibility} visible={state} setVisible={setState} >
      <Stepper steps={ChapterSteps} returnStep={returnCurrentStep}>
        {(() => {
          switch (currentStep) {
            ...
            case 2:
              return <StepTwo mangaId={mangaId} volume={volumeNumber} chapter={chapterNumber} close={toggleVisibility} />
            default:
              return <StepOne returnData={returnData} />
          }
        })()}
